...
async def enrich_batch(limit=20):
    ...
    valid_leads = [
        ...
        if "hiring" not in lead_item["name"].lower()
        and "manager" not in lead_item["name"].lower()
        and "unknown" not in lead_item["name"].lower()
    ]
    ...
    for lead in to_process:
        ...
        if company and company != "Unknown" and len(company) < 50:
            ...
            supabase.table("candidates").update({"company": company}).eq(
                "id", lead["id"]
            ).execute()
            enriched_count += 1
        else:
