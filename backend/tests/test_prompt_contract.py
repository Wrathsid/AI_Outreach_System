...
def test_prompt_section_order():
    ...
    required = {"system_identity", "user_bio", "candidate_context", "structural_rules", "negative_constraints", "task_instruction"}
...
def test_valid_channels():
    ...
    assert VALID_CHANNELS == {"email", "linkedin"}, f"Unexpected channels: {VALID_CHANNELS}"
...
def test_persona_anchor_exists():
    ...
    assert "first person" in PERSONA_ANCHOR.lower() or "you are" in PERSONA_ANCHOR.lower(), \
...
def test_validate_structure():
    ...
    assert validate_structure("Hi\n\nBody here\n\nBest regards", "email") == True
    assert validate_structure("Too short", "email") == False
    
    # LinkedIn needs 2+ paragraphs
    assert validate_structure("Hi there\n\nLet's connect", "linkedin") == True
    assert validate_structure("Single line", "linkedin") == False
...
def test_skills_verifier():
    ...
    assert "Terraform" in result["hallucinated"], "Should detect Terraform as hallucinated"
