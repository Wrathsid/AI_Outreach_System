...
async def upload_avatar(file: UploadFile = File(...)):
    ...
    if supabase:
        try:
            ...
            logger.warning(
                f"Supabase Storage upload failed (falling back to local): {e}"
            )
    ...
    if supabase:
        try:
            ...
            supabase.table("user_settings").update(
                {"avatar_url": None, "updated_at": datetime.now().isoformat()}
            ).eq("id", 1).execute()
        except Exception as e:
...
async def upload_file(file: UploadFile = File(...)):
    ...
    if supabase:
        ...
        if identity.get("full_name") or identity.get("company"):
            try:
                current_settings = (
                    supabase.table("user_settings").select("*").eq("id", 1).execute()
                )
                updates = {}
                current_name = None
                ...
                if updates:
                    ...
                    supabase.table("user_settings").upsert(updates).eq(
                        "id", 1
                    ).execute()
            except Exception as e:
    ...
    return {
        ...
        "preview_text": (
            extracted_text[:200] + "..."
            if extracted_text
            else "No text extracted. (Is this a scanned PDF?)"
        ),
    }
