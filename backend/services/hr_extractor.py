...
def classify_entity(name: str) -> str:
    ...
    """
    if not name: return "PERSON"
    
    # 1. Check for common company suffixes
    company_suffixes = [
        " inc", " llc", " ltd", " corp", " group", " team", " careers", " systems", 
        " technologies", " solutions", " labs", " agency", " studios", " ventures", 
        " capital", " partners", " holdings", " enterprises", " industries", " networks", 
        " software", " consulting", " services", " media", " entertainment", " recruiting",
        " staffing", " talent", " resources", " management", " bank", " global", " international"
    ]
...
def parse_linkedin_title(title: str):
    ...
    normalized = clean_title.replace("–", "-").replace("—", "-").replace("|", "-").replace(" at ", " - at - ") 
...
def parse_linkedin_post_url(url: str) -> Dict[str, str]:
    ...
    try:
        ...
        if "_" in post_part:
            ...
            name_slug = post_part.split("-activity-")[0] if "-activity-" in post_part else post_part
        ...
        for i, part in enumerate(parts):
            ...
            if i > 0 and len(part) >= 6 and all(c in '0123456789abcdef' for c in part.lower()):
...
def extract_poster_info_from_snippet(title: str, body: str) -> Dict[str, Optional[str]]:
    ...
    if body:
        ...
        role_at_match = re.search(r'^([A-Za-z\s]+?)\s+at\s+@?([A-Za-z0-9\s&\.]+?)(?:\s*[·•|]|\s*$)', body)
        if role_at_match:
        ...
        if not result["company"]:
            at_company_match = re.search(r'[Aa]t\s+([A-Z][A-Za-z0-9\s&\.]+?),\s+we', body)
            if at_company_match:
