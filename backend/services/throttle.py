...
class ThrottleService:
    ...
    def record_outcome(cls, outcome: str):
        ...
        if outcome == "error":
            ...
            # Keep last 24h only
            cls._recent_errors = [t for t in cls._recent_errors if t > now - timedelta(hours=24)]
        elif outcome == "bounce":
            cls._recent_bounces.append(now)
            cls._recent_bounces = [t for t in cls._recent_bounces if t > now - timedelta(hours=24)]
        elif outcome == "429":
            cls._last_429 = now
    ...
    def calculate_health_score(cls) -> int:
        ...
        # Each error = -5 points
        err_count = len([t for t in cls._recent_errors if t > now - timedelta(hours=24)])
        score -= (err_count * 5)
        
        # Penalize bounces (last 24h)
        # Each bounce = -10 points (High severity)
        bounce_count = len([t for t in cls._recent_bounces if t > now - timedelta(hours=24)])
